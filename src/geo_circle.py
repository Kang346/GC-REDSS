...
class GeoCircleCalculator:
    ...
    def get_road_network(self, 
        ...
        Args:
            ...
            transport_mode: 'driving', 'walking', 'transit', 'biking', 'subway', or 'all'
        ...
        network_type_map = {
            ...
            "subway": "all",       # Use 'all' to include railway data, then filter for subway
        ...
        cache_key = f"{center_point[0]:.4f}_{center_point[1]:.4f}_{distance}_{network_type}_{transport_mode}"
        ...
        try:
            logger.info(f"Downloading {network_type} network for {center_point} (mode: {transport_mode})")
            ...
            # For subway, use custom filter to get railway/subway network
            if transport_mode == "subway":
                # Custom filter to get subway/railway lines from OpenStreetMap
                # This includes subway, light rail, tram, and rail lines
                custom_filter = '["railway"~"subway|light_rail|tram|rail"]'
                logger.info(f"  Downloading subway/railway network with custom filter...")
                ...
                logger.info(f"  ✓ Subway/railway network downloaded: {len(G.nodes)} nodes, {len(G.edges)} edges")
            else:
                G = ox.graph_from_point(
                    center_point,
                    dist=distance,
            ...
            if network_type in ["drive", "all"]:
                # For subway, add custom speeds (25 km/h average including stops)
                if transport_mode == "subway":
                    logger.info(f"  Setting subway speeds (25 km/h average)...")
                    for u, v, key, data in G.edges(keys=True, data=True):
                        if 'length' in data:
                            length_m = data['length']
                            speed_kmh = 25  # km/h (subway average speed in Manhattan, including stops)
                    ...
                    # For driving and other modes, use standard OSMnx speed calculation
    ...
    def calculate_isochrone(self, 
        ...
        try:
            ...
            MAX_DOWNLOAD_DISTANCE = 100000  # meters (100km - enough for Manhattan area)
            ...
            if transport_mode == "driving":
                ...
                # For subway: use travel_time weight if available (from OSMnx subway network)
                # Fallback to length-based calculation if travel_time not available
                try:
                    # Try to use travel_time weight (preferred, more accurate)
                    cutoff_with_buffer = max_time_seconds * 1.2  # 1.2x buffer
                    path_lengths = nx.single_source_dijkstra_path_length(
                        G, center_node, weight='travel_time', cutoff=cutoff_with_buffer
                    )
                    ...
                    # Fallback: use length and convert to time (25 km/h average subway speed)
                    logger.warning("Subway network missing travel_time, using length-based calculation")
    ...
    def calculate_isochrone_fast(self,
        ...
        try:
            ...
            if transport_mode == "driving":
                ...
                # For subway: use travel_time weight if available (preferred)
                try:
                    path_lengths = nx.single_source_dijkstra_path_length(
                        G, center_node, weight='travel_time', cutoff=max_time_seconds
                    )
                    ...
                    speed_kmh = mode_speeds.get(transport_mode, 25)
